...
function sofass_ocdi_import_files() {
    $demos = array();
    $demos[] = array(
        ...
        'local_import_redux'           => array(
            array(
                ...
                'option_name' => 'sofass_theme_options',
            ),
        ),
        'import_preview_image_url'     => trailingslashit( get_template_directory_uri() ) . 'screenshot.jpg',
        'import_notice'                => esc_html__( 'Import process may take 5-10 minutes. If you facing any issues please contact our support.', 'sofass' ),
        'preview_url'                  => 'http://mygoalthemes.com/demo/sofass/',
    );
    return apply_filters( 'sofass_ocdi_files_args', $demos );
}
add_filter( 'pt-ocdi/import_files', 'sofass_ocdi_import_files' );

function sofass_ocdi_after_import_setup( $selected_import ) {
    ...
    update_option( 'elementor_container_width', 1300 );
    ...
    if ( file_exists($file) ) {
        sofass_ocdi_import_settings($file);
    }

    if ( sofass_is_revslider_activated() ) {
        ...
        $slider_array = array(
            ...
            trailingslashit( get_template_directory() ) . 'inc/vendors/one-click-demo-import/data/slider_4.zip',
            trailingslashit( get_template_directory() ) . 'inc/vendors/one-click-demo-import/data/slider_shop.zip',
        );
...
add_action( 'pt-ocdi/after_import', 'sofass_ocdi_after_import_setup' );

function sofass_ocdi_import_settings($file) {
    ...
    if ( !empty($datas['page_options']) ) {
        sofass_ocdi_import_page_options($datas['page_options']);
    }
    if ( !empty($datas['metadata']) ) {
        sofass_ocdi_import_some_metadatas($datas['metadata']);
    }
}

function sofass_ocdi_import_page_options($datas) {
    if ( $datas ) {
...
function sofass_ocdi_import_some_metadatas($datas) {
    if ( $datas ) {
...
function sofass_ocdi_before_widgets_import() {
...
add_action( 'pt-ocdi/before_widgets_import', 'sofass_ocdi_before_widgets_import' );