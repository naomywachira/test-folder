...
class Sofass_Widget_Woo_Product_Sorting extends WC_Widget {
	...
	public function __construct() {
		...
		$this->widget_description	= esc_html__( 'Display a product sorting list.', 'sofass' );
		$this->widget_id          	= 'goal_woocommerce_widget_product_sorting';
		$this->widget_name        	= esc_html__( 'Goal WooCommerce Product Sorting', 'sofass' );
		$this->settings           	= array(
			'title'  => array(
				'type'  => 'text',
				'std'   => esc_html__( 'Sort By', 'sofass' ),
	...
	public function widget( $args, $instance ) {
		...
		if ( 1 != $wp_query->found_posts || woocommerce_products_will_display() ) {
			...
			$catalog_orderby_options = apply_filters( 'woocommerce_catalog_orderby', array(
				'menu_order'	=> esc_html__( 'Default', 'sofass' ),
				'popularity' 	=> esc_html__( 'Popularity', 'sofass' ),
				'rating'     	=> esc_html__( 'Average rating', 'sofass' ),
				'date'       	=> esc_html__( 'Newness', 'sofass' ),
				'price'      	=> esc_html__( 'Price: Low to High', 'sofass' ),
				'price-desc'	=> esc_html__( 'Price: High to Low', 'sofass' )
			) );
...
if ( function_exists('goal_framework_reg_widget') ) {
    goal_framework_reg_widget('Sofass_Widget_Woo_Product_Sorting');
}