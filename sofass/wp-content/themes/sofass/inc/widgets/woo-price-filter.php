...
class Sofass_Widget_Woo_Price_Filter extends WC_Widget {

	public function __construct() {
		$this->widget_cssclass    = 'goal_widget goal_widget_price_filter woocommerce goal-widget_price_filter';
		$this->widget_description = esc_html__( 'Shows a price filter list in a widget which lets you narrow down the list of shown products when viewing product categories.', 'sofass' );
		$this->widget_id          = 'goal_woocommerce_price_filter';
		$this->widget_name        = esc_html__( 'Goal WooCommerce Price Filter List', 'sofass' );
		$this->settings           = array(
			'title' => array(
				'type'  => 'text',
				'std'   => esc_html__( 'Filter by price', 'sofass' ),
			...
			'price_range_size' => array(
				...
				'max'   => '',
				'std'   => 50,
				'label' => esc_html__( 'Price range size', 'sofass' )
			),
			'max_price_ranges' => array(
				...
				'max'   => '',
				'std'   => 10,
				'label' => esc_html__( 'Max price ranges', 'sofass' )
			),
			'hide_empty_ranges' => array(
				'type'  => 'checkbox',
				'std'   => 1,
				'label' => esc_html__( 'Hide empty price ranges', 'sofass' )
			)
		);
	...
	public function widget( $args, $instance ) {
		...
		if ( strlen( $min_price ) > 0 ) {
			$output .= '<li><a href="' . esc_url( $link ) . '">' . esc_html__( 'All Prices', 'sofass' ) . '</a></li>';
		} else {
            $output .= '<li class="current">' . esc_html__( 'All Prices', 'sofass' ) . '</li>';
		}
...
if ( function_exists('goal_framework_reg_widget') ) {
    goal_framework_reg_widget('Sofass_Widget_Woo_Price_Filter');
}